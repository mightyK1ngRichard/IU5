...
code segment	'code'
	...
	; данные
	ignoredChars 					DB	'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'	;@ список игнорируемых символов
	ignoredLength 				equ	$-ignoredChars				; длина строки ignoredChars
	ignoreEnabled 				DB	0							; флаг функции игнорирования ввода
	translateFrom 				DB	'KVYJG'						;@ символы для замены (АБВГД на англ. раскладке)
	translateTo 				DB	'ЛМНОП'						;@ символы на которые будет идти замена
	...
	charToCursiveIndex 			DB 'И'		      					;@ символ для замены
	...
	signatureLine1				DB	179, 'Пермяков Д. К.                    ', 179
	...
	helpMsg DB '>tsr.com [/?]', 10, 13
			...
			DB '  F9  - вывод ФИО и группы по таймеру в центре экрана', 10, 13
			DB '  F1  - включение и отключения курсивного вывода русского символа И', 10, 13
			DB '  F2  - включение и отключение частичной русификации клавиатуры(KVYJG -> ЛМНОП)', 10, 13
			DB '  F3  - включение и отключение режима блокировки ввода латинских букв', 10, 13
