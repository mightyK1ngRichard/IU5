...
int main()
{
    setlocale(0, "rus"); // русский язык на консоле
    int number_prog;
    cout << "Введите номер задания: ";
    cin >> number_prog;
    switch (number_prog) // переключаем задачи №1-4
    {
        case 1: // задание 1
        {
            bool res = true;
            int n, m, k;
            while (res) // цикл для обеспечения возможности, не завершая программу, найти сумму натуральных чисел 
            {
                ...
                for (int i = 5; i <= n; i += 5)
                {
                    if (i % 5 == 0 && i % m != 0) // если натуральное число удовлетворяет условие, мы суммируем его с k
                        k += i;
                }
        ...
        case 2: // задание 2
        {
            ...
            if (a >= 0)
            {
                for (int i = 2; i <= 8; i += 2) // цикл for с шагом 2
                {
                    s *= i * i;
                }
                s -= a;
        ...
        case 3: // задание 3
        {
            ...
            int n;
            float start = 0.0; float end = 1; float step = 0.2;
        ...
        case 4:  // задание 4
        {
                ...
                bool res = true;
                while (res) // цикл для обеспечения возможности, не завершая программу, вычислять y
                {
                    s1 = s11;
                    cout << "Введите m = ";
                    cin >> m;
                    if (m == 1) { cout << "y= " << sqrt(2) << endl << endl; break; }
                    else {
                        for (int n = 3; n <= m; n++)
                        {
                            s1 = sqrt(2 * n + s1); // новое s1
                            if (n == 3 || n == 5 || n == 10) // условия на промежуточные значения
